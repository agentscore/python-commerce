...
def _input(rails, x402=None) -> BuildPaymentHeadersInput:
    return BuildPaymentHeadersInput(
        rails=rails,
        order_id="ord_1",
...
def test_emits_single_directive_when_one_rail():
    result = build_payment_headers(
        _input([PaymentHeadersRail(rail="tempo-mainnet", amount_usd=10, recipient="0xrecipient")]),
    )
